// origin: pbewithmd5anddes.go
//  https://github.com/LucasSloan/passwordbasedencryption/blob/master/pbewithmd5anddes.go
package main
...
func main() {
  if len(os.Args) != 3 {
   ...
   password := os.Args[1]
   value := os.Args[2]
   data, err := Encrypt(password, 1000, value)
   if err != nil {
	  log.Fatal("failed to encrypt: " + err.Error())
   }  else {
	  fmt.Printf("Result: %s\n" , data)
  }
   result, err := Decrypt(password, 1000, data)
   if err != nil {
	  log.Fatal("failed to decrypt: " + err.Error())
   }  else {
	  fmt.Printf("Result: %s\n" , result)
  }
}
