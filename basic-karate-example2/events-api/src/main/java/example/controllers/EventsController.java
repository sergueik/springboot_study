...
public class EventsController extends ControllerBase {
	...
	public ResponseEntity<APIResponse<List<EventViewModel>>> getEvents(
		...
		try {
			...
			List<EventViewModel> events = eventDAO
					.getEvents(getCurrentUserId(), eventName).stream()
					.map(EventViewModel::fromEvent).collect(Collectors.toList());
			;
			return ResponseEntity.ok(APIResponse.success(events));
		} catch (Exception e) {
			e.printStackTrace();
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.body(APIResponse.<List<EventViewModel>> failure(
							HttpStatus.INTERNAL_SERVER_ERROR.value(), e.getMessage()));
		}
	}

	@GetMapping("/{id}")
	public ResponseEntity<APIResponse<EventViewModel>> getEvent(
			@PathVariable int id) {
		try {
			...
			if (event != null) {
				return ResponseEntity
						.ok(APIResponse.success(EventViewModel.fromEvent(event)));
			} else {
				return ResponseEntity.status(HttpStatus.NOT_FOUND).body(
						APIResponse.<EventViewModel> failure(HttpStatus.NOT_FOUND.value(),
								"Event not found"));
			}
		} catch (Exception e) {
			e.printStackTrace();
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.body(APIResponse.<EventViewModel> failure(
							HttpStatus.INTERNAL_SERVER_ERROR.value(), e.getMessage()));
		}
	}

	@PostMapping("")
	public ResponseEntity<APIResponse<EventViewModel>> addEvent(
			@RequestBody EventDTO eventDTO) {
		try {
			Event newEvent = eventDAO.addEvent(getCurrentUserId(), eventDTO);
			return ResponseEntity.status(HttpStatus.CREATED)
					.body(APIResponse.success(EventViewModel.fromEvent(newEvent)));
		} catch (APIException e) {
			e.printStackTrace();
			return ResponseEntity.status(e.getStatusCode()).body(APIResponse
					.<EventViewModel> failure(e.getStatusCode(), e.getMessage()));
		} catch (Exception e) {
			e.printStackTrace();
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.body(APIResponse.<EventViewModel> failure(
	...
	@PutMapping("/{id}")
	public ResponseEntity<APIResponse<EventViewModel>> updateEvent(
	...
	@DeleteMapping("/{id}")
	public ResponseEntity<APIResponse<EventViewModel>> deleteEvent(
			@PathVariable int id) {
		try {
	...
	public ResponseEntity<APIResponse<List<EventSubscriptionViewModel>>> getSubscriptionsByEvent(
			@PathVariable int id) {
		try {

			List<EventSubscriptionViewModel> events = eventDAO
					.getSubscriptionsByEvent(getCurrentUserId()).stream()
