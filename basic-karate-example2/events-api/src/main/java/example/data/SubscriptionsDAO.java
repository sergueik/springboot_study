...
public class SubscriptionsDAO {
	...
	public List<EventSubscription> getSubscriptionsByUser(int userId) {
		...
		try {
			String hql = "FROM EventSubscription es " + "JOIN FETCH es.event e "
					+ "JOIN FETCH es.userAccount ua " + "WHERE ua.id = :userId";
			Query<EventSubscription> query = session.createQuery(hql,
	...
	public EventSubscription getSubscription(int subId) {
		...
		try {
			String hql = "FROM EventSubscription es " + "JOIN FETCH es.event e "
					+ "JOIN FETCH es.userAccount ua " + "WHERE es.id = :id";
			Query<EventSubscription> query = session.createQuery(hql,
					EventSubscription.class);
	...
	public EventSubscription addSubscription(int userId, int eventId) {
		try (Session session = HibernateUtil.getSessionFactory().openSession()) {
			...
			Query<Event> eventQuery = session.createQuery("FROM Event where id = :id",
					Event.class);
			...
			if (event == null) {
				throw new APIException(404,
						"Failed to add event subscription : Event not found");
			}

			Query<Long> countQuery = session.createQuery(
					"SELECT COUNT(*) FROM EventSubscription WHERE eventId = :eventId",
					Long.class);
			...
			throw new APIException(500,
					"Failed to add subscription: " + e.getMessage());
		}
	}
		...
		try (Session session = HibernateUtil.getSessionFactory().openSession()) {
			...
			if (event.getUserId() != userId) {
				throw new APIException(403,
						"User not allowed to delete this subscription");
			}
