...
public class EventDAO {
	...
	public List<EventSubscription> getSubscriptionsByEvent(int eventId) {
		...
		try {
			String hql = "FROM EventSubscription es " + "JOIN FETCH es.event e "
					+ "JOIN FETCH es.userAccount ua " + "WHERE e.id = :eventId";
		...
		try {
			...
			for (Event event : events) {
				event.setCurrentCapacity(
						event.getMaxCapacity() - event.getEventSubscriptions().size());
			}
	...
	public Event getEvent(int id) {
		...
		try {
			...
			Query<Event> query = session.createQuery("SELECT e FROM Event e "
					+ "LEFT JOIN FETCH e.eventSubscriptions es " + "WHERE e.id = :id",
					Event.class);
