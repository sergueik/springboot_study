...
public final class ViewController {
	...
	public String editNote(@PathVariable int noteId, Model model) {
		...
		model.addAttribute("noteDto",
				new NoteDto(note.get().getId(), note.get().getTitle(), note.get().getText(), note.get().getEmail()));
		return "note";
	}

	@PostMapping(value = "/note/{noteId}")
	public String editNote(@PathVariable int noteId, @ModelAttribute NoteDto noteDto, BindingResult bindingResult,
			Model model) {
