...
class GraphFrame extends JFrame {
	...
	private void createRrdGraph() {
		...
		try (RrdDb rrdDb = RrdDb.getBuilder().setPath(sourcePath).readOnly()
				.build()) {
			RrdDef rrdDef;
			long[] timestamps;
			double[] values;
			...
			LinearInterpolator linearInterpolator = new LinearInterpolator(timestamps,
					values);
			linearInterpolator
					.setInterpolationMethod(LinearInterpolator.Method.LINEAR);
			...
			rrdGraphDef.setFilename(getTempDir() + "/try" + counted++ + ".png");
	...
	private void fillGraphCombo() {
		try (RrdDb rrdDb = RrdDb.getBuilder().setPath(sourcePath).readOnly()
			...
			GraphComboItem[] items = new GraphComboItem[rrdDef.getDsCount()
					* rrdDef.getArcCount()];
			int selectedItem = -1;
	...
	private void changeColor() {
		...
		JColorChooser
				.createDialog(this, "Select color", true, picker, okListener, null)
				.setVisible(true);
	}
	...
	public static String getTempDir() {
		...
		return (defaultTmpDir != null && new File(defaultTmpDir).exists())
				? defaultTmpDir : System.getProperty("user.home");
	}
}
