...
@EnableJpaRepositories(basePackages = { "example.controller", "example.data", "example.repository" })
// trouble when placing Controller and Repository into separate packages
...
public class Launcher {
	...
	public static void main(String[] args) throws Exception {
		 ...
		 * .getResourceAsStream("application.properties"); prop.load(input); } catch
		 ...
		 * "#{username}", "#{password}");
		 * 
		 * Statement statement = null; String query = "select * from #{table}"; try {
		 * statement = connection.createStatement(); ResultSet resultSet =
		 * statement.executeQuery(query); while (resultSet.next()) { String id =
		 * resultSet.getString("ITEM_REFERENCE_ID");
		 * System.out.println("ITEM_REFERENCE_ID: " + id); } } catch (Exception e) {
		 * e.printStackTrace(); } finally { if (statement != null) { statement.close();
		 * } }
		 */

		/*
		 * // occasionally getting org.sqlite.SQLiteException: [SQLITE_ERROR] SQL error
		 * or missing database (no such table: user) the below code creates table
		 * explicitly
		 */

		// DDL to create database table
		/*
		 * System.err.println("Initilizing database"); SessionFactory sessionFactory =
		 * new Configuration().configure().buildSessionFactory(); Session session =
		 * sessionFactory.openSession(); session.beginTransaction(); String query =
		 * String.format(
		 * "CREATE TABLE `user` ( `id`	integer, `nick_name` varchar, `pass_word` varchar, `user_name` varchar, `user_gender` integer, PRIMARY KEY(`id`));"
		 * ); session.createSQLQuery(query); Transaction transaction =
		 * session.getTransaction(); transaction.commit(); session.close();
