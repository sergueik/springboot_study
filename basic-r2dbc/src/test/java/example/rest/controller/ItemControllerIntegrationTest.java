...
public class ItemControllerIntegrationTest {
	...
	void GIVEN_no_item_WHEN_getting_all_THEN_empty_list() {
		...
		webTestClient.get().uri(URI).exchange()
				...
				.expectStatus().isOk().expectBodyList(ItemResource.class)
	...
	void GIVEN_no_item_WHEN_getting_all_THEN_the_item_are_returned() {
		...
		// Given no items
		final Item item1 = itemRepository
				.save(new Item().setStatus(ItemStatus.TODO).setDescription("Item1"))
				.block();
		assertNotNull(item1);

		final Item item2 = itemRepository
				.save(new Item().setStatus(ItemStatus.DONE).setDescription("Item2"))
				.block();
		assertNotNull(item2);
	...
	void GIVEN_a_resource_to_be_created_WHEN_posting_it_THEN_it_is_created() {
		...
		// When
		final String location = webTestClient.post().uri(URI)
				.bodyValue(itemResource).exchange()
				// Then
				.expectStatus().isCreated().expectHeader()
				.valueMatches(HttpHeaders.LOCATION, ".*/items/[0-9]+")
				.returnResult(ResponseEntity.class).getResponseHeaders()
				.get(HttpHeaders.LOCATION).get(0);

		final Long id = Long
				.valueOf(location.substring(location.lastIndexOf('/') + 1));
	...
	void GIVEN_a_resource_to_be_created_without_description_WHEN_posting_it_THEN_it_is_NOT_created() {
		...
		// When
		webTestClient.post().uri(URI).bodyValue(itemResource).exchange()
				// Then
	...
	void GIVEN_no_item_WHEN_trying_to_update_a_non_existing_item_THEN_an_error_is_returned() {
		...
		final ItemUpdateResource itemUpdateResource = new ItemUpdateResource()
				.setDescription("updated").setStatus(ItemStatus.DONE);

		// When
		webTestClient.put().uri(URI + "/-1").bodyValue(itemUpdateResource)
				.header(IF_MATCH, "0").exchange()
				// Then
	...
	void GIVEN_an_item_WHEN_updating_it_THEN_it_is_updated() throws Exception {
		...
		final Item item = itemRepository
				.save(
						new Item().setStatus(ItemStatus.DONE).setDescription("description"))
				.block();
		assertNotNull(item);
		...
		final ItemUpdateResource itemResource = new ItemUpdateResource()
				.setDescription("New description").setStatus(ItemStatus.TODO);

		webTestClient.put().uri(URI + item.getId())
				.header(IF_MATCH, item.getVersion().toString()).bodyValue(itemResource)
				.exchange()
				// Then
	...
	void testOptimisticLockingPut_preconditionFailed() throws Exception {
		...
		// When
		final ItemUpdateResource itemUpdateResource = new ItemUpdateResource()
				.setDescription("update old version").setStatus(ItemStatus.DONE);

		// Precondition failed
		webTestClient.put().uri(URI + item.getId()).bodyValue(itemUpdateResource)
				.header(IF_MATCH, "0").exchange()
				// Then
	...
	void testOptimisticLockingPut_preconditionSucceeded() throws Exception {
		...
		assertEquals(itemUpdateResource.getDescription(),
	...
	@Test
	void GIVEN_no_item_WHEN_trying_to_delete_a_non_existing_item_THEN_an_error_is_returned()
			throws Exception {

		webTestClient.delete().uri(URI + "-1").header(IF_MATCH, "0").exchange()
				// Then
				.expectStatus().isNotFound();
	}
	...
	void GIVEN_an_item_WHEN_deleting_it_THEN_it_is_deleted() throws Exception {
		...
		// When deleting it
		webTestClient.delete().uri(URI + item.getId())
				.header(IF_MATCH, item.getVersion().toString()).exchange()
				// Then
				.expectStatus().isOk();
	...
	void testOptimisticLockingDelete_preconditionFailed() throws Exception {
		...
		// When
		webTestClient.delete().uri(URI + item.getId()).header(IF_MATCH, "1")
				.exchange()
				// Then
				.expectStatus().isEqualTo(HttpStatus.PRECONDITION_FAILED);
	}
	...
	void testOptimisticLockingDelete_preconditionSucceeded() throws Exception {
		...
		// When
		webTestClient.delete().uri(URI + item.getId()).header(IF_MATCH, "0")
				.exchange()
				// Then
				.expectStatus().isOk();
	}
	...
	// -----------------------------------
	@Test
	void GIVEN_no_item_WHEN_trying_to_patch_a_non_existing_item_THEN_an_error_is_returned()
			throws Exception {

		// When
		webTestClient.patch().uri(URI + "-1").header(IF_MATCH, "0")
	...
	@Test
	void GIVEN_an_item_WHEN_patching_its_status_THEN_only_the_status_is_patched()
			throws Exception {
		...
		// When
		webTestClient.patch().uri(URI + item.getId())
	...
	void testOptimisticLockingPatch_preconditionSucceeded() throws Exception {
		...
		// When
		webTestClient.patch().uri(URI + item.getId())
				.bodyValue("{\"description\": \"updated\"}").header(IF_MATCH, "1")
				.header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE)
				.exchange()
				// Then
				.expectStatus().isNoContent();
	...
	private void assertItemEquals(final Item item,
