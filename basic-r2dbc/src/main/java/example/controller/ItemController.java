...
public class ItemController {
	...
	public Mono<ResponseEntity<Void>> update(@PathVariable @NotNull final Long id,
		...
		return itemService.findById(id, version, false).map(item -> itemMapper.update(itemUpdateResource, item))
	...
	public Mono<ResponseEntity<Void>> patch(@PathVariable @NotNull final Long id,
		...
		return itemService.findById(id, version, true).map(item -> itemMapper.patch(patch, item))
				.flatMap(itemService::update).map(itemId -> noContent().build());
	}
	...
	public Flux<ServerSentEvent<Event>> listenToEvents() {
		...
		final Flux<Event> itemSavedFlux = this.itemService.listenToSavedItems().map(itemMapper::toResource)
				.map(ItemSaved::new);

		final Flux<Event> itemDeletedFlux = this.itemService.listenToDeletedItems().map(ItemDeleted::new);

		return Flux.merge(itemSavedFlux, itemDeletedFlux).map(event -> ServerSentEvent.<Event> builder()
				.retry(Duration.ofSeconds(4L)).event(event.getClass().getSimpleName()).data(event).build());
	}
