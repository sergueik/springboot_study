...
public class JavaConsole {
	...
	public static void main(String[] args) {
		...
		Log.setLogger(new LoggerConfiguration().setMinimumLevel(LogEventLevel.Verbose).with(new LogContextEnricher())
				.with(new UserDestructor())
				.writeTo(coloredConsole("[{Timestamp} {Level}] {Message} ({Operation}){NewLine}{Exception}"))
				.writeTo(rollingFile("test-{Date}.log")).writeTo(seq("http://localhost:5341/")).createLogger());
