...
public final class CUrl {
	...
	private static final String DEFAULT_USER_AGENT = "Java-CURL version "
			+ VERSION + " by Rocks Wang(https://github.com/rockswang)";
	private static final Pattern ptnOptionName = Pattern
			.compile("-{1,2}[a-zA-Z][a-zA-Z0-9\\-.]*");
	...
	static {
		try {
			// Try to enable the setting to restricted headers like "Origin", this is
			// expected to be executed before HttpURLConnection class-loading
			System.setProperty("sun.net.http.allowRestrictedHeaders", "true");

			// Modify the system-wide Cookie manager to ThreadLocal-based instance
			CookieManager
					.setDefault(new CookieManager(cookieStore, CookiePolicy.ACCEPT_ALL));
	...
	public final CUrl retry(int retry, float retryDelay, float retryMaxTime) {
		return opt("--retry", Integer.toString(retry), "--retry-delay",
				Float.toString(retryDelay), "--retry-max-time",
				Float.toString(retryMaxTime));
	}
	 ...
	 */
	public final CUrl timeout(float connectTimeoutSeconds,
	...
	public static String dumpCookie(HttpCookie cookie) {
		...
		long expire = cookie.getMaxAge() <= 0
				|| cookie.getMaxAge() >= Integer.MAX_VALUE ? Integer.MAX_VALUE
						: cookie.getMaxAge() + System.currentTimeMillis() / 1000L;
		sb.append(cookie.getDomain()).append('\t').append("FALSE").append('\t')
				.append(cookie.getPath()).append('\t')
				.append(cookie.getSecure() ? "TRUE" : "FALSE").append('\t')
				.append(expire).append('\t').append(cookie.getName()).append('\t')
	...
	public static void loadCookies(IO input) {
		if (input instanceof CookieIO) {
			...
			List<HttpCookie> cookies = parseCookies(
					Util.b2s(readInput(input), null, null));
	...
	public static List<HttpCookie> parseCookies(String input) {
		...
		try {
			for (String line = br.readLine(), l[]; line != null; line = br
					.readLine()) {
				if (line.trim().length() == 0 || line.startsWith("# ")
						|| (l = line.split("\t")).length < 7)
					continue;
				...
				cookie.setMaxAge(expire >= Integer.MAX_VALUE ? Integer.MAX_VALUE
	...
	public final String toString() {
		...
		for (String s : options) {
			sb.append(' ')
					.append(ptnOptionName.matcher(s).matches() ? s : '"' + s + '"');
		}
		if (iomap.size() > 0)
			sb.append("\r\n> IOMap: ").append(iomap);
		return sb.toString();
	...
	public final <T> T exec(Resolver<T> resolver, T fallback) {
		...
		boolean location = false, silent = false, mergeData = false,
				insecure = false;
			...
			case 53: // --data-urlencode
				...
				case -1: // no '='
					if ((atIdx = data.indexOf("@")) >= 0) { // [name]@filename
						...
						try {
							data = prefix + URLEncoder.encode(Util
									.b2s(readInput(getIO(data.substring(atIdx + 1))), null, ""),
									charset);
						} catch (Exception e) {
							lastEx = e;
						}
			...
			case 18: // --proxy-user USER[:PASSWORD] Proxy user and password
				...
				Authenticator.setDefault(new Authenticator() {
					...
					protected PasswordAuthentication getPasswordAuthentication() {
						...
						return new PasswordAuthentication(up[0],
								(up.length > 1 ? up[1] : "").toCharArray());
					}
				});
				break;
		...
		if (dataSb.length() > 0 && form.size() > 0
			...
			lastEx = new IllegalArgumentException(
		...
		if (form.size() > 0) {
			...
			dataStr = !mergeData ? dataSb.toString()
					: Util.join(Util.split(dataSb.toString(), "&", "=",
							new LinkedHashMap<String, String>()), "&", "=");
			if (method == null)
				method = "POST";
		}
		...
		do {
			try {
				...
				if (con instanceof HttpsURLConnection) {
					if (insecure) {
						...
						((HttpsURLConnection) con).setSSLSocketFactory(getSocketFactory(
								getIO(cert.substring(0, idx)), cert.substring(idx + 1)));
					}
				}
				if (verbose) {
						...
						for (Map.Entry<String, Util.Ref<String>> en : form.entrySet()) {
							...
							if (val.getInt() == 1) {
								...
								for (int j = 1; j < ll.length; j++) {
									...
									} else if (ll[j].length > 1
											&& "filename".equals(ll[j][0].trim())) {
				...
				try {
					...
					if (httpCode == 407 && proxy != Proxy.NO_PROXY
							&& "https".equals(urlObj.getProtocol())
							&& headers.containsKey("Proxy-Authorization")) {
						throw new RuntimeException(
								e.getMessage()
										+ "\nTry using VM argument \"-Djdk.http.auth.tunneling.disabledSchemes=\"",
								e);
					}
					if (redirect == null)
						lastEx = e;
				...
				if (maxDownload <= 0 && bb != null && bb.length > 2) {
					...
					if (b0 == 0x78 && ((b1 = bb[1]) == 0x01 || b1 == 0x5E
							|| b1 == (byte) 0x9C || b1 == (byte) 0xDA))
				...
				&& (retryMaxTime <= 0 || System.currentTimeMillis()
						- startTime < (long) (retryMaxTime * 1000d)));
		return error(stdout, stderr, lastEx, silent, resolver, fallback);
	}
	...
	private <T> T error(IO stdout, IO stderr, Throwable ex, boolean silent,
			Resolver<T> rr, T fallback) {
	...
	private static void parseCookies(String url, String input) {
		...
		for (String[] pair : Util.split(input, ";", "=")) {
			HttpCookie cookie = new HttpCookie(pair[0],
					Util.urlDecode(pair[1], "UTF-8"));
	...
	private static void fillResponseHeaders(HttpURLConnection con,
		...
		Integer nkeys;
		if (responses != null
				&& (nkeys = (Integer) Util.getField(responses, null, "nkeys", null,
						true)) != null
				&& (keys = (String[]) Util.getField(responses, null, "keys", null,
						true)) != null
				&& (values = (String[]) Util.getField(responses, null, "values", null,
						true)) != null) {
	...
	private static void dumpHeader(List<String[]> headers, IO dumpHeader)
			throws Exception {
		ByteArrayOutputStream bos = new ByteArrayOutputStream();
		for (String[] kv : headers)
			bos.write(((kv[0] != null ? kv[0] + ": " : "")
					+ (kv[1] != null ? kv[1] : "") + "\r\n").getBytes());
		bos.write(NEWLINE);
	...
	private static void writeOutput(IO out, byte[] bb, boolean append) {
		...
		try {
			...
			Util.logStderr("CUrl.writeOutput: out=%s,bb=%s,append=%s,ex=%s", out, bb,
					append, Util.dumpStackTrace(e, true));
		}
		out.close();
	}

	private static final HashSet<Class> RECOVERABLES = Util.listAdd(
			new HashSet<Class>(), (Class) Recoverable.class, ConnectException.class,
			HttpRetryException.class, SocketException.class,
	...
	private static SSLSocketFactory getSocketFactory(IO cert, String password)
			throws Exception {
		...
		if (cert != null) {
			...
			// use JKS for java, BKS for Android
	...
	public static final ToStringResolver ISO_8859_1 = new ToStringResolver(
			"ISO-8859-1");
	...
	public static class CookieIO implements IO, java.net.CookieStore {
		...
		public void add(URI uri, HttpCookie cookie) {
			...
			Map<String, List<HttpCookie>> map = Util.mapListAdd(getCookiesMap(),
					ArrayList.class, cookie.getDomain());
		...
		private static void normalize(URI uri, HttpCookie cookie) {
			...
			if (Util.empty(cookie.getDomain()))
				throw new IllegalArgumentException(
						"illegal cookie domain: " + cookie.getDomain());
	...
	final static class Util {
		...
		public static String qt(Object o) {
			return o == null || o instanceof Boolean || o instanceof Number ? "" + o
					: o instanceof Character ? "'" + o + "'" : "\"" + o + "\"";
		}
		...
		public static <K, V> Map<K, List<V>> mapListAdd(Map<K, List<V>> map, K key,
				V... val) {
			return mapListAdd(map, ArrayList.class, key, val);
		}

		public static <K, V, L extends Collection<V>> Map<K, L> mapListAdd(
				Map<K, L> map, Class<? extends Collection> collectionClass, K key,
				V... val) {
			L l;
		...
		public static <K, S, V, M extends Map<S, V>> V mapMapGet(Map<K, M> map,
				K key, S subkey, V fallback) {
			M m;
			V ret;
			return (m = map.get(key)) != null && (ret = m.get(subkey)) != null ? ret
					: fallback;
		}
		...
		public static <K, V, M extends Map<K, V>> M mapPut(M map,
				Object... keyValuePairs) {
			if ((keyValuePairs.length & 1) != 0)
				throw new IllegalArgumentException(
						"the number of keyValuePairs arguments must be odd");
		...
		public static class Ref<T> {
			...
			public boolean equals(Object obj) {
				if (!(obj instanceof Ref))
					return false;
				Ref<T> o;
				return (o = (Ref<T>) obj) != null && i == o.i
						&& (v == null ? o.v == null : v.equals(o.v));
			}
		...
		public static String b2s(byte[] bb, int offset, int count, String charset,
				String fallback) {
			try {
				int start = bb.length - offset >= 3 && bb[offset] == 0xEF
						&& bb[offset + 1] == 0xBB && bb[offset + 2] == 0xBF ? 3 : 0; // deal
																																					// BOM
				return new String(bb, offset + start, count - start,
		...
		public static Map<String, String> split(String s, String entryDelim,
				String kvDelim, Map<String, String> toMap) {
		...
		public static String join(Object mapOrColl, String delim, String subDelim) {
			...
			if (mapOrColl == null) { // do nothing
				...
				for (Map.Entry<?, ?> kv : ((Map<?, ?>) mapOrColl).entrySet()) {
					all.add(
							listAdd(new ArrayList<Object>(2), kv.getKey(), kv.getValue()));
				}
			} else if (mapOrColl instanceof Collection) {
				for (Object o : (Collection<?>) mapOrColl)
					all.add(asList(o));
			} else if (mapOrColl.getClass().isArray()) {
				for (int i = 0, n = Array.getLength(mapOrColl); i < n; all
						.add(asList(Array.get(mapOrColl, i++))))
					;
			} else { // plain object
		...
		public static String base64Encode(byte[] bb) {
			...
			if (clz != null) {
				...
				return (String) invokeSilent(encoder, null, "encodeToString", false,
						"[B", (Object) bb);
			}
			clz = getClass("sun.misc.BASE64Encoder", null);
			if (clz != null) {
				Object encoder = createInstance(clz, "", true);
				return ((String) invokeSilent(encoder, null, "encode", true, "[B",
						(Object) bb)).replaceAll("[\r\n]+", "");
			}
			clz = getClass("org.apache.commons.codec.binary.Base64", null);
			if (clz != null) {
				return (String) invokeSilent(null, clz, "encodeBase64String", false,
						"[B", (Object) bb);
			}
			clz = getClass("android.util.Base64", null);
			if (clz != null) {
				return (String) invokeSilent(null, clz, "encodeToString", false, "[BI",
						bb, 2); // NO_WRAP
			}
		...
		public static byte[] readStream(InputStream is, int interruptOnSize,
				boolean close) {
			ByteArrayOutputStream bos = new ByteArrayOutputStream();
			int count = 0, c;
			while ((c = pipeStream(is, bos)) > 0
					&& (interruptOnSize <= 0 || count < interruptOnSize))
				count += c;
			if (c < 0)
		...
		public static int pipeStream(InputStream source, OutputStream destination) {
			...
			do {
				...
				try {
					len = source.read(bb);
				} catch (SocketTimeoutException e) { // no data, but the socket
		...
		public static Class<?> getClass(String className, ClassLoader cl) {
			try {
				return (cl != null ? cl : CUrl.class.getClassLoader())
						.loadClass(className);
		...
		public static <T> T createInstance(Class<T> cls, String signature,
				boolean ignoreAccess, Object... args) {
		...
		public static Object getField(Object thiz, Class<?> cls, String fieldName,
				Object fallback, boolean ignoreAccess) {
			if (thiz == null && cls == null || fieldName == null)
				throw new NullPointerException(
						"inst=" + thiz + ",class=" + cls + ",field=" + fieldName);
			try {
				for (MemberInfo mi : safeIter(
						getMembers(thiz != null ? thiz.getClass() : cls, fieldName))) {
					if (-1 == mi.numArgs && (ignoreAccess
							|| (mi.member.getModifiers() & Modifier.PUBLIC) != 0)) {
						AccessibleObject acc;
						if (ignoreAccess
								&& !(acc = (AccessibleObject) mi.member).isAccessible())
		...
		public static Object invokeSilent(Object thiz, Class<?> cls,
				String methodName, boolean ignoreAccess, String signature,
				Object... args) {
			try {
		...
		public static Object invoke(Object thiz, Class<?> cls, String methodName,
				boolean ignoreAccess, String signature, Object... args) {
			if (thiz == null && cls == null || methodName == null)
				throw new NullPointerException(
						"inst=" + thiz + ",class=" + cls + ",method=" + methodName);
			List<MemberInfo> found = getMembers(thiz != null ? thiz.getClass() : cls,
					methodName);
			try {
				Member m = null;
				if (found == null) { // do nothing
					...
					for (MemberInfo mi : found) {
						if (len == mi.numArgs && (ignoreAccess
								|| (mi.member.getModifiers() & Modifier.PUBLIC) != 0)) {
							m = mi.member;
							break;
						}
					...
					for (MemberInfo mi : found) {
						if (signature.equals(mi.signature) && (ignoreAccess
								|| (mi.member.getModifiers() & Modifier.PUBLIC) != 0)) {
							m = mi.member;
							break;
						}
				...
				if (m == null) {
					StringBuilder msg = new StringBuilder().append('"').append(methodName)
							.append('"');
					if (signature == null) {
				...
				if (ignoreAccess && !(acc = (AccessibleObject) m).isAccessible())
					acc.setAccessible(true);
				return m instanceof Method ? ((Method) m).invoke(thiz, args)
						: ((Constructor<?>) m).newInstance(args);
			} catch (Exception ex) {
				throw new IllegalArgumentException(ex);
			}
		}

		private static final Map<String, Object> primaryTypes = newMap(byte.class,
				'B', char.class, 'C', double.class, 'D', float.class, 'F', int.class,
				'I', long.class, 'J', short.class, 'S', void.class, 'V', boolean.class,
				'Z');
		...
		private static synchronized List<MemberInfo> getMembers(Class<?> cls,
				String name) {
			if (!mapClassMembers.containsKey(cls)) {
				Map<String, List<MemberInfo>> map;
				mapClassMembers.put(cls,
						map = new LinkedHashMap<String, List<MemberInfo>>());
				Class<?> clss = cls;
				while (clss != null && !Object.class.equals(clss)) {
					for (Constructor<?> c : safeArray(clss.getDeclaredConstructors(),
							Constructor.class)) {
						Class<?>[] ptypes = c.getParameterTypes();
						mapListAdd(map, "<init>",
								new MemberInfo(getSignature(ptypes), ptypes.length, c));
					}
					for (Method m : safeArray(clss.getDeclaredMethods(), Method.class)) {
						Class<?>[] ptypes = m.getParameterTypes();
						mapListAdd(map, m.getName(),
								new MemberInfo(getSignature(ptypes), ptypes.length, m));
					}
