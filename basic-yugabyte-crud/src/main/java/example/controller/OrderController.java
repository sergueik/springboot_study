...
public class OrderController {
	...
	public Order createOrder(@Valid @RequestBody Order order) {
		...
		for (Order.Product orderProduct : order.getProducts()) {
			orderTotal += productRepository.findById(orderProduct.getProductId())
					.map(product -> {
						return product.getPrice() * orderProduct.getUnits();
					}).orElseThrow(() -> new ResourceNotFoundException(
							"Product not found with Id: " + orderProduct.getProductId()));
		}
		...
		order.setUser(userRepository.findById(order.getUserId()).map(user -> {
			return user;
				...
				"UserId " + order.getUserId() + " not found")));

		Order newOrder = orderRepository.save(order);

		for (Order.Product orderProduct : order.getProducts()) {
			orderLineRepository.save(new OrderLine(newOrder.getOrderId(),
					orderProduct.getProductId(), orderProduct.getUnits()));
		}
	...
	public Order updateOrder(@PathVariable UUID orderId,
		...
		for (Order.Product orderProduct : orderRequest.getProducts()) {
			orderLineRepository.save(new OrderLine(orderId,
					orderProduct.getProductId(), orderProduct.getUnits()));
		}
		...
		return orderRepository.findById(orderId).map(order -> {
			...
			order.setUser(
					userRepository.findById(orderRequest.getUserId()).map(user -> {
						return user;
					}).orElseThrow(() -> new ResourceNotFoundException(
							"UserId " + order.getUserId() + " not found")));
