...
> **Stop.** In a low-code agent, the runtime calls these endpoints when the built-in `deep-rag` tool is enabled — see [impl-json.md](impl-json.md). Direct API calls are debug-only.
