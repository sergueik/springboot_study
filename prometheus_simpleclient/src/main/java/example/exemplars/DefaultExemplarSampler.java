...
public class DefaultExemplarSampler implements ExemplarSampler {
	...
	// Choosing a prime number for the retention interval makes behavior more
	// predictable,
	// because it is unlikely that retention happens at the exact same time as a
	// Prometheus scrape.
	...
	private Exemplar doSample(double value, Exemplar previous) {
		...
		if (previous == null || previous.getTimestampMs() == null
			...
			if (traceId != null && spanId != null) {
				return new Exemplar(value, timestampMs, SPAN_ID, spanId, TRACE_ID,
						traceId);
			}
		}
		return null;
