...
public class Enumeration extends SimpleCollector<Enumeration.Child>
		...
		for (String label : labelNames) {
			if (label.equals(fullname)) {
				throw new IllegalStateException(
						"Enumeration cannot have a label named the same as its metric name.");
			}
		}
		states = b.states;
	...
	public static class Builder
		...
		public Enumeration create() {
			if (states == null) {
				throw new IllegalStateException(
						"Enumeration states must be specified.");
			}
			if (!unit.isEmpty()) {
				throw new IllegalStateException(
						"Enumeration metrics cannot have a unit.");
			}
	...
	public List<MetricFamilySamples> collect() {
		...
		for (Map.Entry<List<String>, Child> c : children.entrySet()) {
			...
			for (String s : states) {
				...
				samples
						.add(new MetricFamilySamples.Sample(fullname, labelNamesWithState,
								labelValuesWithState, s.equals(v) ? 1.0 : 0.0));
			}
		}
	...
	public List<MetricFamilySamples> describe() {
		return Collections
				.singletonList(new MetricFamilySamples(fullname, Type.STATE_SET, help,
