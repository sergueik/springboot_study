...
public class CKMSQuantilesTest {
	...
	public void testBatchInsert() {
		...
		testInsertBatch(2, 110, 100, random); // compress never called, because
																					// compress interval > number of
																					// inserts
	...
	private void testInsertBatch(int batchSize, int compressInterval,
			int totalNumber, Random random) {
		System.out.println(
				"testInsertBatch(batchSize=" + batchSize + ", compressInterval="
						+ compressInterval + ", totalNumber=" + totalNumber + ")");
		...
		for (int i = 0; i < input.size(); i += batchSize) {
			...
			validateSamples(ckms); // after each insert the samples should still be
															// valid
			insertsSinceCompress += j;
			if (insertsSinceCompress >= compressInterval) {
				ckms.compress();
				validateSamples(ckms); // after each compress the samples should still
																// be valid
	...
	public void testGetWithAMillionElements() {
		...
		validateResults(ckms);
		assertTrue("sample size should be way below 1_000_000",
				ckms.samples.size() < 1000);
	}
	...
	public void testMinAndOthers() {
		...
		validateResults(ckms);
		assertTrue(ckms.samples.size() < 200); // should be a lot less than
																						// input.size()
	}
	...
	public void testMaxEpsilon() {
		...
		List<Double> input = shuffledValues(10000, random);
		// epsilon == 1 basically gives you random results, but it should still not
		// throw an exception.
	...
	public void testGetGaussian() {
		...
		double mean = 0.0;
		double stddev = 1.0;
		NormalDistribution normalDistribution = new NormalDistribution(rand, mean,
				stddev, NormalDistribution.DEFAULT_INVERSE_ABSOLUTE_ACCURACY);
		...
		assertEquals(p10, ckms.get(0.1),
				errorBoundsNormalDistribution(0.1, 0.001, normalDistribution));
		assertEquals(mean, ckms.get(0.5),
				errorBoundsNormalDistribution(0.5, 0.01, normalDistribution));
		assertEquals(p90, ckms.get(0.9),
				errorBoundsNormalDistribution(0.9, 0.001, normalDistribution));
		assertEquals(p95, ckms.get(0.95),
				errorBoundsNormalDistribution(0.95, 0.001, normalDistribution));
		assertEquals(p99, ckms.get(0.99),
				errorBoundsNormalDistribution(0.99, 0.001, normalDistribution));

		assertTrue("sample size should be below 1000", ckms.samples.size() < 1000);
	}

	double errorBoundsNormalDistribution(double p, double epsilon,
		...
		// subtract and divide by 2, assuming that the increase is linear in this
		// small epsilon.
	...
	private void validateSamples(CKMSQuantiles ckms) {
		...
		for (CKMSQuantiles.Sample sample : ckms.samples) {
			String msg = "invalid sample " + sample + ": count=" + ckms.n + " r=" + r
					+ " f(r)=" + ckms.f(r);
		...
		assertEquals("the sum of all g's must be the total number of observations",
				r, ckms.n);
	}
	...
	private void validateResults(CKMSQuantiles ckms) {
		for (Quantile q : ckms.quantiles) {
			...
			String errorMessage = q + ": " + actual + " not in [" + lowerBound + ", "
					+ upperBound + "], n=" + ckms.n + ", " + q.quantile + "*" + ckms.n
					+ "=" + (q.quantile * ckms.n);
