...
import example.exemplars.DefaultExemplarSampler;
...
public class DefaultExemplarSamplerTest {
	...
	public void testCounter() {
		...
		Exemplar first = new Exemplar(2.0, timestamp.get(), TRACE_ID, traceId.get(),
				SPAN_ID, spanId.get());
		Assert.assertEquals(first, sampler.sample(2.0, null));
		traceId.set("trace-2");
		spanId.set("span-2");
		timestamp.getAndAdd(100); // 100ms later
		Exemplar second = new Exemplar(3.0, timestamp.get(), TRACE_ID,
				traceId.get(), SPAN_ID, spanId.get());
		...
		traceId.set("trace-3");
		spanId.set("span-3");
		Exemplar third = new Exemplar(4.0, timestamp.get(), TRACE_ID, traceId.get(),
				SPAN_ID, spanId.get());
	...
	public void testHistogram() {
		...
		// Almost identical to Counter and Gauge, except that the value is passed
		// directly and not via valueHolder.
		Exemplar first = new Exemplar(2.0, timestamp.get(), TRACE_ID, traceId.get(),
				SPAN_ID, spanId.get());
		Assert.assertEquals(first,
				sampler.sample(2.0, NEGATIVE_INFINITY, POSITIVE_INFINITY, null));
		traceId.set("trace-2");
		spanId.set("span-2");
		timestamp.getAndAdd(100); // 100ms later
		Exemplar second = new Exemplar(3.0, timestamp.get(), TRACE_ID,
				traceId.get(), SPAN_ID, spanId.get());
		Assert.assertNull(
				sampler.sample(3.0, NEGATIVE_INFINITY, POSITIVE_INFINITY, first)); // no
																																						// new
																																						// exemplar
																																						// yet
		Assert.assertEquals(second,
				sampler.sample(3.0, NEGATIVE_INFINITY, POSITIVE_INFINITY, null));
		timestamp.getAndAdd(10 * 1000); // 10s later
		traceId.set("trace-3");
		spanId.set("span-3");
		Exemplar third = new Exemplar(4.0, timestamp.get(), TRACE_ID, traceId.get(),
		...
		Assert.assertEquals(third,
				sampler.sample(4.0, NEGATIVE_INFINITY, POSITIVE_INFINITY, second));
		traceId.set(null);
		Assert.assertNull(
				sampler.sample(4.0, NEGATIVE_INFINITY, POSITIVE_INFINITY, null));
	}
}
