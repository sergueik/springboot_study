...
public class SimpleCollectorTest {
	...
	public void setUp() {
		...
		metric = Gauge.build().name("labels").help("help").labelNames("l")
				.register(registry);
	...
	public void testNameIsConcatenated() {
		assertEquals("a_b_c", Gauge.build().name("c").subsystem("b").namespace("a")
				.help("h").create().fullname);
	}
	...
	public void testUnitsAdded() {
		...
		Gauge g2 = Gauge.build().name("a_seconds").unit("seconds").help("h")
				.create();
	...
	public void testCreateReturnsGauge() {
		Gauge g = Gauge.build().name("labels").help("help").labelNames("l")
				.create();
	}
}
