...
import example.Collector;
import example.CollectorRegistry;
import example.Counter;
import example.Gauge;
import example.GaugeMetricFamily;
import example.Histogram;
import example.SampleNameFilter;
...
public class SampleNameFilterTest {
	...
	public void testCounter() {
		// It should not make any difference whether a counter is created as
		// "my_counter" or as "my_counter_total".
		// We test both ways here and expect the same output.
		for (String suffix : new String[] { "", "_total" }) {
			registry.clear();
			Counter counter1 = Counter.build().name("counter1" + suffix)
					.help("test counter 1").labelNames("path").register(registry);
			counter1.labels("/hello").inc();
			counter1.labels("/goodbye").inc();
			Counter counter2 = Counter.build().name("counter2" + suffix)
					.help("test counter 2").register(registry);
			counter2.inc();

			SampleNameFilter filter = new SampleNameFilter.Builder().build();
			List<Collector.MetricFamilySamples> mfsList = Collections
					.list(registry.filteredMetricFamilySamples(filter));
			...
			filter = new SampleNameFilter.Builder().nameMustStartWith("counter1")
					.build();
			...
			filter = new SampleNameFilter.Builder().nameMustNotStartWith("counter1")
					.build();
			mfsList = Collections.list(registry.filteredMetricFamilySamples(filter));
			assertSamplesInclude(mfsList, "counter2_total", 1);
			assertSamplesInclude(mfsList, "counter2_created", 1);
			...
			filter = new SampleNameFilter.Builder()
					...
					.nameMustBeEqualTo("counter1_total").build();
			mfsList = Collections.list(registry.filteredMetricFamilySamples(filter));
			assertSamplesInclude(mfsList, "counter1_total", 2);
			assertSamplesInclude(mfsList, "counter2_total", 1);
			...
			filter = new SampleNameFilter.Builder().nameMustStartWith("counter1")
					.nameMustNotStartWith("counter1_created").build();
			mfsList = Collections.list(registry.filteredMetricFamilySamples(filter));
			assertSamplesInclude(mfsList, "counter1_total", 2);
			assertTotalNumberOfSamples(mfsList, 2);

			// The following filter would be weird in practice, but let's test this
			// anyways :)
			filter = new SampleNameFilter.Builder()
					.nameMustBeEqualTo("counter1_created")
					.nameMustBeEqualTo("counter2_created")
					.nameMustNotStartWith("counter1").build();
			mfsList = Collections.list(registry.filteredMetricFamilySamples(filter));
			assertSamplesInclude(mfsList, "counter2_created", 1);
			...
			filter = new SampleNameFilter.Builder()
					...
					.nameMustNotBeEqualTo("counter1_total").build();
			mfsList = Collections.list(registry.filteredMetricFamilySamples(filter));
	...
	public void testCustomCollector() {
		Collector myCollector = new Collector() {
			...
			public List<MetricFamilySamples> collect() {
				...
				GaugeMetricFamily temperatureCentigrade = new GaugeMetricFamily(name,
						"temperature centigrade", labelNames);
				temperatureCentigrade.samples.add(new MetricFamilySamples.Sample(name,
						labelNames, Collections.singletonList("outside"), 26.0));
				temperatureCentigrade.samples.add(new MetricFamilySamples.Sample(name,
						labelNames, Collections.singletonList("inside"), 22.0));
				result.add(temperatureCentigrade);

				name = "temperature_fahrenheit";
				GaugeMetricFamily temperatureFahrenheit = new GaugeMetricFamily(name,
						"temperature fahrenheit", labelNames);
				temperatureFahrenheit.samples.add(new MetricFamilySamples.Sample(name,
						labelNames, Collections.singletonList("outside"), 78.8));
				temperatureFahrenheit.samples.add(new MetricFamilySamples.Sample(name,
						labelNames, Collections.singletonList("inside"), 71.6));
		...
		SampleNameFilter filter = new SampleNameFilter.Builder()
				.nameMustStartWith("temperature_centigrade").build();
		List<Collector.MetricFamilySamples> mfsList = Collections
				.list(registry.filteredMetricFamilySamples(filter));
		...
		filter = new SampleNameFilter.Builder()
				.nameMustNotStartWith("temperature_centigrade").build();
		mfsList = Collections.list(registry.filteredMetricFamilySamples(filter));
		assertSamplesInclude(mfsList, "temperature_fahrenheit", 2);
		assertTotalNumberOfSamples(mfsList, 2);

		filter = new SampleNameFilter.Builder().nameMustNotStartWith("temperature")
				.build();
		mfsList = Collections.list(registry.filteredMetricFamilySamples(filter));
		assertTotalNumberOfSamples(mfsList, 0);

		filter = new SampleNameFilter.Builder()
				.nameMustBeEqualTo("temperature_centigrade").build();
		mfsList = Collections.list(registry.filteredMetricFamilySamples(filter));
		assertSamplesInclude(mfsList, "temperature_centigrade", 2);
		assertTotalNumberOfSamples(mfsList, 2);

		filter = new SampleNameFilter.Builder()
				.nameMustNotBeEqualTo("temperature_centigrade").build();
		mfsList = Collections.list(registry.filteredMetricFamilySamples(filter));
		assertSamplesInclude(mfsList, "temperature_fahrenheit", 2);
		assertTotalNumberOfSamples(mfsList, 2);

		filter = new SampleNameFilter.Builder().nameMustStartWith("temperature_c")
				.build();
		mfsList = Collections.list(registry.filteredMetricFamilySamples(filter));
		assertSamplesInclude(mfsList, "temperature_centigrade", 2);
		assertTotalNumberOfSamples(mfsList, 2);
	...
	public void testHistogram() {
		Histogram histogram = Histogram.build().name("test_histogram")
				.help("test histogram").create();
		...
		filter = new SampleNameFilter.Builder().nameMustStartWith("test_histogram")
				// nameMustStartWith() is for the names[] query parameter in HTTP
				// exporters.
				// If histogram_created is missing in the names[] list, it should not be
				// exported
				...
				.nameMustBeEqualTo("test_histogram_sum").build();
		mfsList = Collections.list(registry.filteredMetricFamilySamples(filter));
		assertSamplesInclude(mfsList, "test_histogram_bucket", 15);
		assertSamplesInclude(mfsList, "test_histogram_count", 1);
		assertSamplesInclude(mfsList, "test_histogram_sum", 1);
		...
		filter = new SampleNameFilter.Builder().nameMustStartWith("test_histogram")
				...
				.nameMustNotStartWith("test_histogram_bucket").build();
		mfsList = Collections.list(registry.filteredMetricFamilySamples(filter));
		assertSamplesInclude(mfsList, "test_histogram_count", 1);
		assertSamplesInclude(mfsList, "test_histogram_sum", 1);
		assertSamplesInclude(mfsList, "test_histogram_created", 1);
	...
	public void testLegacyApi() {
		...
		PartiallyFilterCollector pfr = new PartiallyFilterCollector()
				.register(registry);
		HashSet<String> metrics = new HashSet<String>();
		HashSet<String> series = new HashSet<String>();
		Set<String> includedNames = new HashSet<String>(Arrays.asList("", "s_sum",
				"c_total", "part_filter_a", "part_filter_c"));
		List<Collector.MetricFamilySamples> mfsList = Collections
				.list(registry.filteredMetricFamilySamples(includedNames));
		...
		assertEquals(
				new HashSet<String>(
						Arrays.asList("s", "c", "part_filter_a", "part_filter_c")),
				metrics);
		assertEquals(new HashSet<String>(
				Arrays.asList("s_sum", "c_total", "part_filter_a", "part_filter_c")),
				series);
	}
	...
	private static class SkippedCollector extends Collector
	...
	private static class PartiallyFilterCollector extends Collector
			implements Collector.Describable {
		public int collectCallCount = 0;
	...
	private void assertSamplesInclude(List<Collector.MetricFamilySamples> mfsList,
	...
	private void assertTotalNumberOfSamples(
			List<Collector.MetricFamilySamples> mfsList, int n) {
		int count = 0;
		for (Collector.MetricFamilySamples mfs : mfsList) {
