...
public class CounterTest {
	...
	public void setUp() {
		...
		labels = Counter.build().name("labels").unit("seconds").help("help")
				.labelNames("l").register(registry);
	}
	...
	private Double getLabelsValue(String labelValue) {
		return registry.getSampleValue("labels_seconds_total", new String[] { "l" },
	...
	public void testTotalStrippedFromName() {
		Counter c = Counter.build().name("foo_total").unit("seconds").help("h")
				.create();
	...
	public void testCollect() {
		...
		labelValues.add("a");
		samples.add(new Collector.MetricFamilySamples.Sample("labels_seconds_total",
				labelNames, labelValues, 1.0));
		samples
				.add(new Collector.MetricFamilySamples.Sample("labels_seconds_created",
						labelNames, labelValues, labels.labels("a").created() / 1000.0));
		Collector.MetricFamilySamples mfsFixture = new Collector.MetricFamilySamples(
				"labels_seconds", "seconds", Collector.Type.COUNTER, "help", samples);
	...
	public void testExemplars() {
		...
		noLabels.incWithExemplar(5, (Map<String, String>) null); // should not alter
																															// the exemplar
		assertExemplar(noLabels, 5, "mapKey1", "mapValue1", "mapKey2", "mapValue2");
	}
