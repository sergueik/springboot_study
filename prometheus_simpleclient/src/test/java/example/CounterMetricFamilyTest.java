...
import example.CounterMetricFamily;
...
public class CounterMetricFamilyTest {
	...
	public void testJavadocExample() {
		class YourCustomCollector extends Collector {
			public List<MetricFamilySamples> collect() {
				...
				// With labels
				CounterMetricFamily labeledCounter = new CounterMetricFamily(
						"my_other_counter", "help", Arrays.asList("labelname"));
		...
		assertEquals(42.0,
				registry.getSampleValue("my_counter_total").doubleValue(), .001);
		assertEquals(null, registry.getSampleValue("my_other_counter_total"));
		assertEquals(4.0,
				registry
						.getSampleValue("my_other_counter_total",
		...
		assertEquals(5.0,
				registry
						.getSampleValue("my_other_counter_total",
								new String[] { "labelname" }, new String[] { "bar" })
						.doubleValue(),
				.001);
	}
	...
	public void testBuilderStyleUsage() {
		...
		assertEquals(1.0, registry.getSampleValue("my_metric_total",
				new String[] { "name" }, new String[] { "value1" }).doubleValue(),
				.001);
		assertEquals(2.0, registry.getSampleValue("my_metric_total",
	...
	public void testTotalHandling() {
		...
		assertEquals(1.0, registry.getSampleValue("a_total",
				new String[] { "name" }, new String[] { "value" }).doubleValue(), .001);
		assertEquals(2.0, registry.getSampleValue("b_total",
				new String[] { "name" }, new String[] { "value" }).doubleValue(), .001);
