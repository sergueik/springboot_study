...
import com.mycila.event.SendableRequest;
...
public final class ComTest {
	...
	public void test_async_request() throws Exception {
		...
		SendableRequest<Integer> req = processor
				.createRequestor(topic("system/add"))
				.<Integer> createRequest("my sum", new int[] { 1, 2, 3, 4, 5 })
		...
		SendableRequest<Integer> req2 = processor
				.createRequestor(topic("system/rm")).<Integer> createRequest("err")
				.addListener(new FutureListener<Integer>() {
					public void onResponse(Integer value) {
						fail();
					}
	...
	public void test() throws Exception {
		...
		dispatcher.subscribe(only("system/df"), EventRequest.class,
				new Subscriber<EventRequest<Integer>>() {
					public void onEvent(Event<EventRequest<Integer>> event)
							throws Exception {
						...
						if ("inexisting".equals(folder))
							event.getSource().replyError(new FileNotFoundException(
									"df did not found folder " + folder));
						else
		...
		// manually
		SendableRequest<Integer> req1 = processor
				.createRequestor(topic("system/df")).createRequest("home");
		SendableRequest<Integer> req2 = processor
				.createRequestor(topic("system/df")).createRequest("inexisting");
	...
	static abstract class DU {
		...
		void duRequest(Event<EventRequest<Integer>> event) {
			...
			if ("notFound".equals(folder))
				event.getSource().replyError(
						new FileNotFoundException("du did not found folder " + folder));
			else
