...
final class PerfTest {
	...
	private static final ConcurrentLinkedQueue<StatEvent> events = new ConcurrentLinkedQueue<>();
	...
	public static void main(String... args) throws Exception {
		// NOTE: '<>' cannot be used with anonymous classes

		Map<String, Dispatcher> dispatchers = new LinkedHashMap<String, Dispatcher>(
				6) {
			{
				put("SynchronousSafe",
						Dispatchers.synchronousSafe(ErrorHandlers.rethrow()));
				put("SynchronousUnsafe",
						Dispatchers.synchronousUnsafe(ErrorHandlers.rethrow()));
				put("AsynchronousSafe",
						Dispatchers.asynchronousSafe(ErrorHandlers.rethrow()));
				put("AsynchronousUnsafe",
						Dispatchers.asynchronousUnsafe(ErrorHandlers.rethrow()));
				put("BroadcastOrdered",
						Dispatchers.broadcastOrdered(ErrorHandlers.rethrow()));
				put("BroadcastUnordered",
						Dispatchers.broadcastUnordered(ErrorHandlers.rethrow()));
			}
		};
		for (Map.Entry<String, Dispatcher> entry : dispatchers.entrySet()) {
			...
			for (int i = 0, length = N_SUBS * 2; i < length; i++) {
				...
				entry.getValue().subscribe(Topic.only("stats" + index), StatEvent.class,
						new Subscriber<StatEvent>() {
	...
	static void test(final Dispatcher dispatcher, int nPublishers, int nEvents)
			throws Exception {
		System.out.println("\n > Testing " + nPublishers + " publishers sending "
				+ nEvents + " events each\n");
		...
		System.out.println("- publish time: " + (end - start) + "ns = "
				+ TimeUnit.MILLISECONDS.convert(end - start, TimeUnit.NANOSECONDS)
				+ "ms");
		System.out.println("- avg. latency: " + latency + "ns = "
				+ TimeUnit.MILLISECONDS.convert(latency, TimeUnit.NANOSECONDS) + "ms");
		System.out.println("- min. latency: " + min + "ns = "
				+ TimeUnit.MILLISECONDS.convert(min, TimeUnit.NANOSECONDS) + "ms");
		System.out.println("- max. latency: " + max + "ns = "
				+ TimeUnit.MILLISECONDS.convert(max, TimeUnit.NANOSECONDS) + "ms");
	}

	private static void publish(final Dispatcher dispatcher,
			final int nPublishers, final int nEvents) throws Exception {
				...
				public void run() {
					try {
						...
						for (int i = 0; i < nEvents; i++) {
							dispatcher.publish(Topic.topic("stats" + random.nextInt(N_SUBS)),
									new StatEvent());
