...
Scenario: Obtain access token
  ...
  * def parts_length = parts.length

  * match parts_length == 3
  ...
  * def headerJsonString = decode(parts[0])
  * def headerJson = karate.fromString(headerJsonString)
  * def methods = getMethods(headerJson)
  * print 'Methods of the object:', methods
  * match headerJson contains { alg: 'HS256' }

  # * print headerJson
  * def payloadJson = karate.fromString(decode(parts[1]))
  * print payloadJson
