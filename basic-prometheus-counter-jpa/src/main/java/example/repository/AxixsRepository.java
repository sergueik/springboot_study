...
public interface AxixsRepository
	...
	// NOTE: use SQL notation: consult the class field mapping in the model
	// package
	...
	@Query(nativeQuery = true, value = "SELECT sname AS hostname,'dummy' AS dc,aname AS application, iname AS env from axixs x join server s on x.sid = s.sid left join application a on x.aid = a.aid join instance i on x.iid = i.iid")
	public List<Object[]> findAllDataNative();

	@Query(nativeQuery = true, value = "SELECT sname AS hostname,'dummy' AS dc,aname AS application, iname AS env from axixs x join server s on x.sid = s.sid left join application a on x.aid = a.aid join instance i on x.iid = i.iid WHERE REGEXP_LIKE(sname, ?1 )")
	public List<Object[]> findFilteredDataNative(String serverNamesRegexp);
