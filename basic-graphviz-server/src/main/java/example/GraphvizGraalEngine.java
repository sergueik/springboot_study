...
public class GraphvizGraalEngine implements AutoCloseable {
	...
	public GraphvizGraalEngine() {
		try {
			...
			context = Context.newBuilder("js").allowAllAccess(true).option("engine.WarnInterpreterOnly", "false")
					.build();

			loadJsResource(
					Config.get("libraryPath", "/META-INF/resources/webjars/viz.js-graphviz-java/2.1.3") + "/viz.js");
			loadJsResource(Config.get("libraryPath", "/META-INF/resources/webjars/viz.js-graphviz-java/2.1.3")
					+ "/full.render.js");
	...
	/**
	 * Render DOT -> SVG or PNG
	 */
	public byte[] renderDot(String dot, String format) {
		try {
			String js = """
			...
			Value promise = renderFunc.execute(dot);

			String svg = awaitPromise(promise);

			log.info("SVG generated, length={}", svg.length());

			return "svg".equals(format) ? svg.getBytes() : svgToPng(svg);
	...
	private byte[] svgToPng(String svg) {
		try {
			...
			baos.flush();
			log.info("PNG generated, length={}", baos.toByteArray().length);
