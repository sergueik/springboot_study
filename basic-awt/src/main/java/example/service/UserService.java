...
import example.dto.response.JwtTokenResponse;
import example.entity.User;
import example.exception.ValidationResponseException;
import example.security.JwtTokenProvider;
import example.security.UserPrincipal;
...
public class UserService {
	...
	public JwtTokenResponse login(String email, String password) {
		userRepository.findByEmail(email).map(User::getPassword)
				.filter(passHash -> passwordEncoder.matches(password, passHash))
				.orElseThrow(
						() -> new ValidationResponseException("Invalid credentials"));
	...
	public void delete(String email) {
		User user = this.findByEmail(email)
				.orElseThrow(() -> new ValidationResponseException(
	...
	public User update(UserDto userToUpdate) {
		this.findByEmail(userToUpdate.getEmail())
				.orElseThrow(() -> new ValidationResponseException(
						"User with email " + userToUpdate.getEmail() + " does not exists"));
