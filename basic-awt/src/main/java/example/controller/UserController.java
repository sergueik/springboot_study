...
import example.dto.request.LoginRequest;
import example.dto.request.RegistrationRequest;
...
public class UserController {
	...
	public UserDto register(@Valid @RequestBody RegistrationRequest request) {
		return UserDto.from(userService.register(request.getEmail(),
				request.getName(), request.getPassword()));
	}

	@PostMapping("token/refresh")
	public JwtTokenResponse refresh(
	...
	public UserDto findByEmail(@PathVariable String email) {
		return userService.findByEmail(email).map(UserDto::from)
