	...
	public static void main(String[] args) throws Exception {
		...
		if (benchmark) {
			Options options = new OptionsBuilder()
					...
					// NOTE: params must match the properties of the class 
					.output("benchmark_results.txt").param("copybookFile", copybookFile).param("inputFile", inputFile)
					.param("maxRows", String.format("%d", maxRows)).param("page", page).build();
			System.err.println(String.format(
					"run with options: fork: %d warmup: %d measurement: %d copybookFile: %s inputFile: %s maxRows: %s",
					options.getForkCount().get(), options.getWarmupIterations().get(),
					options.getMeasurementIterations().get(), options.getParameter("copybookFile").get(),
