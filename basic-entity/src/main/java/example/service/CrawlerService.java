...
public class CrawlerService {
	...
	public void crawl(String url) {
		...
		while (!urlQueue.isEmpty()) {
			...
			try {
				...
				String content = document.select("meta[property=og:type]")
						.attr("content");
	...
	private void checkWeatherCrawlingIsFinished() {
		if (urlQueue.size() == 0) {
			...
			System.out.println(
					"All the gathered information are saved in products.db database");
		}
	}
	...
	private void getPrice(Document document, Map<String, String> productInfoMap) {
		String price = document.select("div.product-info-main")
				.select("div.product-info-price").select("span.price").text();
		productInfoMap.put(PRICE, price);
	}

	private void getDescription(Document document,
			Map<String, String> productInfoMap) {
		String description = document.select("div.product.attribute.description")
				.text();
		productInfoMap.put(DESCRIPTION, description);
	}

	private void getExtraInformation(Document document,
			Map<String, String> productInfoMap) {
		StringBuilder extraInformation = new StringBuilder();
		String style = document.getElementById("product-attribute-specs-table")
				.select("td[data-th=Style]").text();
		String material = document.getElementById("product-attribute-specs-table")
				.select("td[data-th=Material]").text();
		String pattern = document.getElementById("product-attribute-specs-table")
				.select("td[data-th=Pattern]").text();
		String climate = document.getElementById("product-attribute-specs-table")
				.select("td[data-th=Climate]").text();

		extraInformation.append("Style  ").append(style).append("\nMaterial  ")
