...
def render_screen( screen, output_file, font, cols=DEFAULT_COLS, rows=DEFAULT_ROWS, left=DEFAULT_LEFT, top=DEFAULT_TOP, foreground="lime", background="black", character_granular=False,):
    """
    ...
    actual_cols = max( [cols] + [len(line) for line in screen])
...
def create_labels( screen, output_file, cols, rows, font_path, font_size, foreground, background, character_granular,):
    """
    ...
    labels = { "cols": cols, "rows": rows, "font": str(font_path), "font_size": font_size, "foreground": foreground, "background": background, "character_granular": character_granular, "screen": screen, "text": "\n".join(screen), }
...
def parse_args():
    parser = argparse.ArgumentParser( description=( "Render a text screen specification " "as ASCII and/or PNG."))

    parser.add_argument( "--screenfile", required=True, help="Input screen text file")

    parser.add_argument( "--outputfile", default=None, help="Output PNG file")

    parser.add_argument( "--font", default=None, help=( "TrueType/OpenType font. " "Defaults to FONT_PATH or platform default."))
    ...
    parser.add_argument( "--foreground", default="lime")

    parser.add_argument( "--background", default="black")

    parser.add_argument( "--character-granular", action="store_true", help="Draw each character separately")

    parser.add_argument( "--passthrough", action="store_true", help=( "Print the screen text. " "Can be used without PNG rendering.")
    )

    parser.add_argument( "--textfile", default=None, help="Optional ASCII/text output file")

    parser.add_argument( "--labelfile", default=None, help="Optional JSON ground-truth label file")
...
def main():
    ...
    screen = read_screen( args.screenfile)
    #
    # ASCII passthrough.
    #
    ...
    if args.textfile:
        ...
        print_screen( screen, args.textfile)
    ...
    if args.outputfile:
        ...
        font_path = ( Path(args.font) if args.font else get_default_font_path())

        if args.debug:

            print( f"Font: {font_path}", file=sys.stderr)

            print( f"Screen rows: {len(screen)}", file=sys.stderr)

            print( "Maximum columns: " f"{max(map(len, screen), default=0)}", file=sys.stderr)

        font = load_font( font_path, args.font_size)

        image = render_screen( screen=screen, output_file=args.outputfile, font=font, cols=args.cols, rows=args.rows, left=args.left, top=args.top, foreground=args.foreground, background=args.background, character_granular=args.character_granular,)

        print( f"Wrote {args.outputfile}")

        if args.debug:

            print( f"Image size: " f"{image.width}x{image.height}", file=sys.stderr)
        ...
        if args.labelfile:
            ...
            create_labels( screen=screen, output_file=args.labelfile, cols=args.cols, rows=args.rows, font_path=font_path, font_size=args.font_size, foreground=args.foreground, background=args.background, character_granular=args.character_granular,)

            print( f"Wrote {args.labelfile}")
    ...
    #
        ...
        print( "Nothing to do. " "Specify --outputfile, --passthrough, or --textfile.", file=sys.stderr)
