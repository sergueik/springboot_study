package example.config.prometheus;
...
public class PushGatewayConfiguration {
	...
	public MeterRegistryCustomizer<MeterRegistry> metricsCommonTags(
			@Value("${spring.profiles.active:default}") String activeEnvProfile) {
		return registry -> registry.config().commonTags("env", activeEnvProfile);
	}
	...
	public PrometheusPushGatewayManager prometheusPushGatewayManager(
		...
		PrometheusPushGatewayManager.ShutdownOperation shutdownOperation = properties
				.getShutdownOperation();
		return new PrometheusPushGatewayManager(
				this.getPushGateway(properties.getBaseUrl(), credentials),
				collectorRegistry, pushRate, job, groupingKey, shutdownOperation);
	}
	...
	private static String getJob(PrometheusProperties.Pushgateway properties,
