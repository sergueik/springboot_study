...
public class Vault {
	 ...
	 * Initialize Vault
	 *
	 * @param vaultServer http address to vault server, example:
	 *                    http://127.0.0.1:8200
	...
	/**
	 * Initialize Vault with custom Jersey HTTP client, for fine tuning and proxy
	 * configurations
	 *
	 * @param vaultServer http address to vault server, example:
	 *                    http://127.0.0.1:8200
	 ...
	 * @param secret A key, paired with an associated value, to be held at the given
	 *               location. Multiple key/value pairs can be specified, and all
	 *               will be returned on a read operation. The generic backend use
	 *               'value' as key
	 ...
	 * @param parameters A key, paired with an associated value, to be held at the
	...
	public void delete(String path) {
		...
		Response response = null;
		try {
			response = target.request().accept("application/json").header("X-Vault-Token", this.vaultToken).delete();
	...
	public VaultStatus getStatus() {
		...
		Response response = null;
		try {
			response = target.request().accept("application/json").header("X-Vault-Token", this.vaultToken).get();
			if (response.getStatus() != 200) {
				ErrorResponse error = response.readEntity(ErrorResponse.class);
				throw new VaultException(response.getStatus(), error.getErrors());
			}
	 ...
	 *                           It's easiest to use the TokenCreateRequestBuilder
	 *                           to generate request.
